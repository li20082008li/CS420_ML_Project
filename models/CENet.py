...
class RMPblock(nn.Module):
    ...
    def forward(self, x):
        ...
        layer1 = F.interpolate(self.conv(self.pool1(x)), size=(h, w), mode='bilinear')
        layer2 = F.interpolate(self.conv(self.pool2(x)), size=(h, w), mode='bilinear')
        layer3 = F.interpolate(self.conv(self.pool3(x)), size=(h, w), mode='bilinear')
        layer4 = F.interpolate(self.conv(self.pool4(x)), size=(h, w), mode='bilinear')
